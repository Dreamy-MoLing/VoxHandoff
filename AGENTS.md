...
VoxHandoff is a voice relay for local and remote Hermes sessions. It records locally, produces editable text, sends only confirmed text to an explicitly selected Hermes session, preserves the complete reply, and optionally speaks a separate short summary. Hermes is the only product-supported Agent; Codex research code is historical regression coverage and OpenClaw work is paused.
