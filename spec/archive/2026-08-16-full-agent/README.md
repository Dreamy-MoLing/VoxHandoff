...
> 当前产品支持范围为 Android（当前优先）、iOS（后续）、macOS 和 Windows；
> Linux 仅作为 Hermes/Agent/服务端部署主机，不提供客户端。2026-08-13 起重新
> 开启有限产品路线，本轮只推进 Android 手机端 MVP；macOS/Windows 后续按同一
> 顺序适配，iOS 在后续移动端阶段适配。归档内容仍作为历史证据保留；后台监听、
> 唤醒词、全双工语音、本地手机 Agent/STT sidecar 和新 Agent 后端均不在当前批次。
